...
| **Verified cross-references (source-backed)** | Every pair cites the official document that states it (ABB competitor lookup, Eaton UL-classified charts, Signify/Keystone ballast guides, Hubbell cross guide, Siemens QD bulletin…): explainable match reason, attribute agreement/conflicts, source-derived warnings, confidence by source authority, <95% suppressed from production. Brand hierarchy (51 sourced brand→parent entries) and a provenance gate (≥95 confidence required; unsourced records quarantined) back the verified catalog — see [source-registry.md](source-registry.md) and the auto-regenerated [data-quality-report.md](data-quality-report.md) |
