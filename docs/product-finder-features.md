...
| Feature | Summary |
|---|---|
| **API rate limiting** | Fixed-window per-caller limiter (`lib/server/rate-limit.ts`, tested) on **every cost- and write-sensitive route across Waves 1–4** — assistant, crosses, BOM analyze, RFQ, jobs, orders, VMI, supplier bids, procurement (CIF/PunchOut), commodity, SSO — see the authoritative per-route table in [docs/product-finder-api.md](product-finder-api.md#rate-limiting). Over-limit returns `429` with `Retry-After` + `X-RateLimit-*` headers. Backed by **Upstash Redis** (global, cross-instance) when configured, else an in-memory per-instance fallback |
...
| **Vendor-managed inventory (VMI)** | Set per-SKU **min/max** stocking policies (📦 VMI toolbar / Ctrl+K) and get a live **replenishment** view: each policy pairs current **on-hand** stock (catalog) against **projected 30-day demand** (summed from the durable order history) to compute what's `available` and a recommended **reorder quantity** that restocks to max — flagged `OK / Reorder / Critical`, worst first. One click drafts the replenishment through the same durable order path. Server-persisted via `/api/vmi`; the reorder math is a pure tested lib (`lib/product-finder-vmi.ts`). Turns the recommender into a managed-replenishment partner, not just a search tool. |
| **Supplier collaboration portal** | The sell-side counterpart to the inbound RFQ: a standalone **[supplier portal](/product-finder/supplier)** where a supplier browses the open RFQs the distributor logged and submits a **priced, lead-timed bid**; bids are **ranked best-first** (lowest total, lead time breaks ties) for the rep to compare. One bid per `(rfqRef, supplier)` — resubmitting revises it. Server-persisted via `POST/GET/DELETE /api/rfq-responses`; the totals + ranking are a pure tested lib (`lib/product-finder-supplier.ts`). Closes the loop from "customer RFQ in" to "supplier quotes back," all on the durable store. |
| **CIF catalog + PunchOut Level 2** | Upgrades the procurement integration to enterprise grade. **`GET /api/procurement/cif`** emits a standard **CIF 3.0** flat-file catalog (Ariba/SAP) — each row carrying the item's deep-link URL. **`POST /api/procurement/punchout`** is the cXML PunchOut *setup* endpoint: a `PunchOutSetupRequest` carrying a `<SelectedItem>` returns a StartPage that **deep-links straight to that product** (**Level 2** item-level punchout, `X-PunchOut-Level: 2`) instead of just the store home (Level 1). Both are pure tested libs (`lib/procurement/cif.ts`, `punchout-setup.ts`), extending the existing cart-return cXML + EDI 850 export. The integration depth large procurement orgs require before go-live. |
