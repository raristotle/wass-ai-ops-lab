...
| **Self-service returns / RMA** | A **Start a return** action on any order: pick lines + a reason, get an **RMA number** + estimated credit, and track status (Requested → Approved → In-transit → Received → Credit issued) — surfaced in the 🔔 notification bell until resolved (`lib/product-finder-returns.ts`). Rounds out the post-purchase loop; a Resend key sends the customer an RMA email. |
| **Persistence + queue seam (env-gated, dormant)** | The durable-backend foundation: a namespaced server store (`lib/server/persistence.ts` — Postgres/Prisma when `POSTGRES_URL` is set, per-instance memory otherwise) and a job-queue seam (`lib/server/queue.ts` — inline today, BullMQ worker when `REDIS_URL` is set), with `database`/`queue` booleans on `/api/health` and a `PersistedRecord` Prisma model. Ships **dormant at zero cost** — the app keeps its current localStorage behavior in the browser until infra is wired; quotes/orders/RMAs/RFQ intakes upgrade to durable server storage when it is. |
