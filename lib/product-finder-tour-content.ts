...
export const TOUR_STEPS: readonly TourStep[] = [
  ...
  {
    id: "close-the-deal",
    title: "Close the deal — e-signature & rebates",
    body: [
      "Two things help a quote actually close:",
      "• ✍️ Send for signature emails the customer a Dropbox Sign link to sign and accept — a legally-binding close. It defaults to free test mode and stays hidden until your admin sets the Dropbox key, so there's no cost to try it.",
      "• Open any LED fixture or lamp and you'll see an estimated utility rebate (per fixture and for your quantity) — often the number that wins a lighting retrofit. It's grounded in real utility programs and clearly labeled an estimate the local utility confirms.",
      "When a customer signs, the signature is captured server-side and logged to the tamper-evident audit trail automatically.",
    ],
    action: { kind: "openCart", label: "Open the cart" },
  },
  {
    id: "insights",
    title: "Insights & coaching for managers",
    body: [
      "Managers and admins get an Insights dashboard that opens with coaching:",
      "• Next Best Actions — one ranked “do this next” list (counter-offers, approvals, stale quotes, at-risk accounts, unclaimed rebates, demand to stock), each deep-linking to where you act.",
      ...
      "• Pricing win/loss by margin band, customer health, and a 30-day branch demand forecast.",
      "• A tamper-evident Audit Log — hash-chained activity with a live integrity badge and a one-click CSV export for compliance.",
      "• Catalog Data Quality — every product is scored 0-100 on completeness, with the average, tier mix, and biggest gaps so the catalog team knows what to enrich.",
      "• Will-call queue — when orders are staged for branch pickup, a card opens the queue to print pick tickets.",
