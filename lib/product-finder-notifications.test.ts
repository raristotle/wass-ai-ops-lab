...
describe("buildNotifications — approvals", () => {
  it("emits approval notifications for pending quotes when manager", () => {
    ...
    const out = buildNotifications({ quotes: [q], watches: [], orders: [], customers: [], isManager: true }, NOW);
  ...
  it("suppresses approval notifications for sales role", () => {
    ...
    const out = buildNotifications({ quotes: [q], watches: [], orders: [], customers: [], isManager: false }, NOW);
    expect(out).toHaveLength(0);
  });

  it("ignores approved/rejected quotes", () => {
    const out = buildNotifications(
      { quotes: [quote({ approvalStatus: "approved" }), quote({ id: "q2", approvalStatus: "rejected" })], watches: [], orders: [], customers: [], isManager: true },
      NOW
    );
    expect(out).toHaveLength(0);
  });
...
describe("buildNotifications — stale quotes", () => {
  ...
  it("does not flag fresh sent quotes or non-sent quotes", () => {
    ...
    const out = buildNotifications({ quotes: [fresh, draft], watches: [], orders: [], customers: [], isManager: false }, NOW);
    expect(out).toHaveLength(0);
  });
});
...
describe("buildNotifications — watches", () => {
  ...
  it("young watch → restock-eta with the due date in the detail", () => {
    const out = buildNotifications({ quotes: [], watches: [w], orders: [], customers: [], isManager: false }, NOW + DAY);
    expect(out).toHaveLength(1);
  ...
  it("elapsed watch → restock-due, timed at the due moment", () => {
    ...
    const out = buildNotifications({ quotes: [], watches: [w], orders: [], customers: [], isManager: false }, NOW + (days + 1) * DAY);
    expect(out).toHaveLength(1);
...
describe("buildNotifications — counter-offers", () => {
  it("emits a counter notification for open countered quotes (any role)", () => {
    const q = quote({ status: "sent", counterOffer: { note: "Can you do $60 even?", at: NOW - DAY } });
    const out = buildNotifications({ quotes: [q], watches: [], orders: [], customers: [], isManager: false }, NOW);
    expect(out).toHaveLength(1);
    expect(out[0].id).toBe("counter:quote-1");
    expect(out[0].kind).toBe("counter");
    expect(out[0].detail).toContain("Can you do $60 even?");
    expect(out[0].at).toBe(NOW - DAY);
  });

  it("suppresses counter notifications once the quote is decided", () => {
    const won = quote({ status: "won", counterOffer: { note: "x", at: NOW } });
    const lost = quote({ id: "q2", status: "lost", counterOffer: { note: "x", at: NOW } });
    const out = buildNotifications({ quotes: [won, lost], watches: [], orders: [], customers: [], isManager: false }, NOW);
    expect(out).toHaveLength(0);
  });

  it("truncates long counter notes in the detail", () => {
    const q = quote({ status: "sent", counterOffer: { note: "y".repeat(120), at: NOW } });
    const [n] = buildNotifications({ quotes: [q], watches: [], orders: [], customers: [], isManager: false }, NOW);
    expect(n.detail.length).toBeLessThan(120);
    expect(n.detail).toContain("…");
  });
});

describe("buildNotifications — customer risk", () => {
  const customers = [{ id: "CUST-001", name: "Gulf Coast Industrial" }];
  const orderAt = (daysAgo: number) => ({
    id: `o-${daysAgo}`,
  ...
  it("emits customer-risk for accounts gone quiet beyond 2× cadence", () => {
    // single order 120 days ago → way past the 45-day baseline × 2
    const out = buildNotifications(
      { quotes: [], watches: [], orders: [orderAt(120)], customers, isManager: false },
      NOW
    );
    expect(out).toHaveLength(1);
    expect(out[0].id).toBe("customer-risk:CUST-001");
    expect(out[0].kind).toBe("customer-risk");
    expect(out[0].customerId).toBe("CUST-001");
    expect(out[0].title).toContain("Gulf Coast");
  });

  it("stays silent for healthy accounts", () => {
    const out = buildNotifications(
      { quotes: [], watches: [], orders: [orderAt(35), orderAt(5)], customers, isManager: false },
      NOW
    );
    expect(out).toHaveLength(0);
  });
...
describe("buildNotifications — ordering & unread", () => {
  it("sorts newest first across kinds", () => {
    ...
    const out = buildNotifications({ quotes: [stale, pending], watches: [w], orders: [], customers: [], isManager: true }, NOW);
  ...
  it("unreadCount counts only ids missing from the read map", () => {
    ...
    const out = buildNotifications({ quotes: [pending], watches: [{ id: "p1", name: "X", addedAt: NOW }], orders: [], customers: [], isManager: true }, NOW);
